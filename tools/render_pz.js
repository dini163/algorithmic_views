...
['core.js', 'util.js', 'engines.js', 'desc.js', 'idea.js', 'data_o.js', 'data_a.js', 'data_b.js', 'data_c.js', 'desc_m.js', 'idea_m.js', 'data_m.js', 'desc_p.js', 'idea_p.js', 'data_p.js']
  .forEach(function (f) {
...
function findDef(no) {
  const s = String(no);
  const pre = s[0];
  const isO = pre === 'o', isM = pre === 'm', isP = pre === 'p';
  const n = parseInt((isO || isM || isP) ? s.slice(1) : s, 10);
  return PZ.defs.find(function (d) {
    return d.no === n && (isM ? d.g === 'm' : isO ? d.g === 'o' : isP ? d.g === 'p' : d.g !== 'o' && d.g !== 'm' && d.g !== 'p');
  });
}
...
argNo.split(',').forEach(function (no) {
  ...
  const M = PZ.wrapModel(PZ.engines[d.e].build(d.p || {}), d);
  const key = (d.g === 'o' ? 'o' : d.g === 'm' ? 'm' : d.g === 'p' ? 'p' : '') + d.no + '_' + d.e;
  let ks;
