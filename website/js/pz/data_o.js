...
(function () {
  ...
  D({ g: g, no: 6, title: '变位词检测', e: 'board', strat: '变治',
    plain: '判断两个词是不是"字母打乱重排"的关系。笨办法要枚举所有排列；聪明办法：把字母排序后直接比字符串。',
    p: { steps: [
      { cap: 'listen 和 silent 是变位词吗？', fn: function (ctx, W) { U.row(ctx, W, 90, 'listen'.split('')); U.row(ctx, W, 150, 'silent'.split('')); } },
      { cap: '变治：把问题变成"排序后比较"', fn: function (ctx, W) { U.lines(ctx, W, [['分别按字母排序 ↓', 15, '#8fa0c8']], 40); U.row(ctx, W, 90, 'eilnst'.split(''), [0, 1, 2, 3, 4, 5]); U.row(ctx, W, 150, 'eilnst'.split(''), [0, 1, 2, 3, 4, 5]); } },
      { cap: '排序结果完全相同 → 是变位词 ✓（O(n log n) 代替 O(n!)）', fn: function (ctx, W) { U.row(ctx, W, 100, 'eilnst'.split(''), [0, 1, 2, 3, 4, 5]); U.lines(ctx, W, [['完全一致 ✓', 18, '#4ade80', true]], 180); } }
    ] } });

  /* 概7 现金分装 */
  D({ g: g, no: 7, title: '现金分装', e: 'board', strat: '数学技巧·二进制',
    plain: '把一百万现金分装进信封，要求任何金额都能整信封给出。按 2 的幂装：1,2,4,8…，任何数都有唯一的二进制拆法。',
    p: { steps: [
      { cap: '$1,000,000 分装信封：任意金额都能整封支付', fn: function (ctx, W) { U.lines(ctx, W, [['$1,000,000 → ? 个信封', 18, '#5eead4', true]], 120); } },
      { cap: '信封按 2 的幂装：1, 2, 4, 8, 16, …, 524288（共 20 个）', fn: function (ctx, W) { U.row(ctx, W, 100, [1, 2, 4, 8, 16, 32, 64, '…', 524288]); U.lines(ctx, W, [['每个数字的二进制表示唯一', 14, '#8fa0c8']], 180); } },
      { cap: '取 $100 = 64 + 32 + 4 → 只给 3 个信封', fn: function (ctx, W) { U.row(ctx, W, 100, [1, 2, 4, 8, 16, 32, 64, '…'], [2, 5, 6]); U.lines(ctx, W, [['$100 = 64 + 32 + 4 ✓', 18, '#fbbf24', true]], 180); } }
    ] } });
  ...
  D({ g: g, no: 9, title: 'Guarini 谜题', e: 'gridmove', strat: '图论·状态空间',
    plain: '3×3 棋盘四角各有一位骑士，白在上黑在下，要让黑白互换位置。把骑士所有合法落点连成一个 8 步循环，大家沿循环各走 4 步即可，共 16 跳。',
    p: { rows: 3, cols: 3, pieces: [
      { id: 'W1', label: '白1', color: '#e2e8f0', r: 0, c: 0 }, { id: 'W2', label: '白2', color: '#e2e8f0', r: 0, c: 2 },
      { id: 'B1', label: '黑1', color: '#334155', r: 2, c: 0 }, { id: 'B2', label: '黑2', color: '#334155', r: 2, c: 2 }],
      moves: guaMoves, cap: '16 步沿"8 字循环"轮换完成换位' } });
  ...
  D({ g: g, no: 10, title: '最优馅饼切法', e: 'geo', strat: '数学技巧',
    plain: '直直地切 k 刀，最多能把馅饼切成几块？每新的一刀都要和之前所有刀痕相交：答案是 1 + k(k+1)/2。',
    p: { steps: [
  ...
  D({ g: g, no: 13, title: '柠檬水摊设点', e: 'board', strat: '贪心·中位数',
    plain: '一条街上有几户人家，柠檬水摊摆在哪让大家走的总路最短？不是平均位置，而是中位数位置——两边户数平衡的点。',
    p: { steps: [
      { cap: '房子在 1, 3, 4, 9, 14 号位置，摊位摆哪？', fn: function (ctx, W) { U.axis(ctx, W, 150, 0, 15, [1, 3, 4, 9, 14]); } },
      { cap: '摆两头会让远处的人走很多冤枉路', fn: function (ctx, W) { U.axis(ctx, W, 150, 0, 15, [1, 3, 4, 9, 14], [{ v: 14, color: '#f87171', label: '太远' }]); } },
      { cap: '中位数 4 号：左边 2 户右边 2 户，总距离最小 ✓', fn: function (ctx, W) { U.axis(ctx, W, 150, 0, 15, [1, 3, 4, 9, 14], [{ v: 4, color: '#4ade80', label: '最优' }]); } }
    ] } });

  /* 概14 正数变号 */
  D({ g: g, no: 14, title: '正数变号', e: 'board', strat: '数学技巧',
    plain: '给 1~8 各配上正号或负号，让总和恰好为 0。总和 36 是偶数，挑出和为 18 的一组数变负号即可。',
    p: { steps: [
      { cap: '给 1~8 配正负号，使总和 = 0', fn: function (ctx, W) { U.row(ctx, W, 120, [1, 2, 3, 4, 5, 6, 7, 8]); } },
      { cap: '总和 36 → 变负号的那组数之和必须是 36÷2 = 18', fn: function (ctx, W) { U.row(ctx, W, 120, [1, 2, 3, 4, 5, 6, 7, 8], [2, 6, 7]); } },
      { cap: '取 3+7+8 = 18 → +1+2−3+4+5+6−7−8 = 0 ✓', fn: function (ctx, W) { U.row(ctx, W, 120, ['+1', '+2', '−3', '+4', '+5', '+6', '−7', '−8'], [2, 6, 7]); } }
    ] } });
  ...
  /* 概17 方块搭建 */
  D({ g: g, no: 17, title: '方块搭建', e: 'geo', strat: '数学技巧·调和级数',
    plain: '一摞方块能伸出桌边多远？从上往下第 k 块相对下一块外移 1/(2k) 个块长，总外移 1/2+1/4+1/6+…，想伸多远就叠多少块。',
    p: { steps: [1, 2, 3, 4].map(function (k) {
      return { cap: '叠 ' + k + ' 块：顶部相对底部外移 ' + ['1/2', '1/2+1/4', '1/2+1/4+1/6', '1/2+1/4+1/6+1/8'][k - 1] + ' 个块长',
        fn: function (ctx, W, Hh) {
          var L = 110, bh = 34, baseY = 260, cx = W / 2 - 60;
          H.line(ctx, cx + L / 2, baseY + 8, cx + L / 2, baseY + 60, '#f87171', 2);
          H.txt(ctx, '桌边', cx + L / 2 + 30, baseY + 50, { size: 11, color: '#f87171' });
          var shift = 0;
          for (var i = k; i >= 1; i--) {
            var y = baseY - (k - i + 1) * bh;
            ctx.fillStyle = H.PAL[i % 10];
            H.rr(ctx, cx + shift, y, L, bh - 4, 5); ctx.fill();
            shift += L / (2 * (i));
          }
        } };
    }) } });
  ...
  /* 概21 田地里的鸡 */
  D({ g: g, no: 21, title: '田地里的鸡', e: 'board', strat: '几何·不变量',
    plain: '正方形田地要用一条直线篱笆分成面积相等的两半，篱笆必须经过田的中心——任何过中心的直线都恰好把正方形平分。',
    p: { steps: [
      { cap: '一条直线把正方形田地分成面积相等的两半', fn: function (ctx, W, Hh) {
        var s = 170, x0 = (W - s) / 2, y0 = (Hh - s) / 2 - 8;
        ctx.strokeStyle = '#5eead4'; ctx.lineWidth = 2; ctx.strokeRect(x0, y0, s, s); } },
      { cap: '中心对称图形：任何过中心的直线都平分面积', fn: function (ctx, W, Hh) {
        var s = 170, x0 = (W - s) / 2, y0 = (Hh - s) / 2 - 8, cx = x0 + s / 2, cy = y0 + s / 2;
        ctx.strokeStyle = '#5eead4'; ctx.lineWidth = 2; ctx.strokeRect(x0, y0, s, s);
        H.circle(ctx, cx, cy, 5, '#fbbf24');
        H.line(ctx, cx - s * 0.7, cy - s * 0.35, cx + s * 0.7, cy + s * 0.35, '#f87171', 2); } },
      { cap: '对角线、横竖中线…过中心的篱笆都是最短且公平的', fn: function (ctx, W, Hh) {
        var s = 170, x0 = (W - s) / 2, y0 = (Hh - s) / 2 - 8, cx = x0 + s / 2, cy = y0 + s / 2;
        ctx.strokeStyle = '#5eead4'; ctx.lineWidth = 2; ctx.strokeRect(x0, y0, s, s);
        H.line(ctx, x0, y0, x0 + s, y0 + s, '#f87171', 2);
        H.line(ctx, cx, y0, cx, y0 + s, '#4ade80', 2);
        H.circle(ctx, cx, cy, 5, '#fbbf24'); } }
    ] } });
})();
